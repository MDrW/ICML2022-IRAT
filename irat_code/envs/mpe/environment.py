...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 team_reward_callback=None, sparse_reward=False,
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            ...
            idv_rew = self._get_reward(agent)
            reward_n.append([idv_rew])
            done_n.append(self._get_done(agent))
            info = {'individual_reward': idv_rew}
            env_info = self._get_info(agent)
            if 'fail' in env_info.keys():
                info['fail'] = env_info['fail']
            if 'detail_infos' in env_info.keys():
                info['detail_infos'] = env_info['detail_infos']
            if 'additional_infos' in env_info.keys():
                info['additional_infos'] = env_info['additional_infos']
            if 'catch_infos' in env_info.keys():
                info['catch_infos'] = env_info['catch_infos']
            info_n.append(info)

        # all agents get total reward in cooperative case,
        # if shared reward, all agents have the same reward, and reward is sum
        if self.team_reward_callback is not None:
            team_rew = self.team_reward_callback(self.world, self.sparse_reward)
        else:
            team_rew = np.sum(reward_n)

        if self.shared_reward:
            reward_n = [[team_rew]] * self.n

        if self.reward_shaping:
            reward_n = [[team_rew + irw[0]] for irw in reward_n]

        for i in range(len(info_n)):
            info_n[i]["team_reward"] = team_rew
